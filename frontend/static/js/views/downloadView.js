...
export default class DownloadView extends AbstractView {
    ...
    async getHtml() {
        return `
            <div class="container-download100" style="background-image: url('static/css/images/bg-01.jpg');">
                <div class="wrap-download100 p-t-30 p-b-50 download-form">
                    <span class="download100-form-title p-b-41">
                        Download File
                    </span>
    ...
    async executeViewScript() {
        ...
        // Display files and setup event listeners
        await displayFiles();
        setupEventListeners();
        ...
        async function downloadFile(fileName, fileOwner) {
            ...
            // Assemble file content from server blocks
            const fileData = await assembleFileContent();
        
            // Hide loader after download completion
        ...
        function assembleFileContent() {
            return new Promise((resolve, reject) => {
                ...
                // Listen for file blocks from the server
                socket.on('fileBlock', async (fileBlockPayload) => {
                    ...
                    // Store total blocks count if not already stored
                    if (totalBlocks === -1) {
                        totalBlocks = currentTotalBlocks;
                    }
        
                    // Append block to fileData and check for completion
